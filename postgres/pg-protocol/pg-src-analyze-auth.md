...
实际在使用PostgreSQL数据库时，都会对用户进行认证，只有合法用户才会允许访问数据库，这里分析口令认证的过程。口令认证的大致过程，用户连接数据库，数据库postmaster进程监听到客户端的请求，fork一个postgres子进程负责与客户端进行交互，postgres子进程首先会解析startup消息，也就是从客户端收到的第一个消息，然后根据协议以及认证方式进行认证，认证通过后才会向客户端发送`ReadyForQuery`消息，表示服务端准备好接收客户端的请求了，postgres子进程进入主循环，等待用户输入命令，执行命令。
